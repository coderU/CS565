...
Definition blt_nat (n m : nat) : bool :=
  match n with
    ...
    | S n' =>
      match m with
        | O => false
        | S m' => andb (orb (beq_nat n' m') (ble_nat n' m')) (negb (andb (beq_nat n' m') (ble_nat n' m')))
      end
  end.
...
Eval simpl in increment ZERO. (** 1 *)
Eval simpl in increment (increment ZERO). (** 2 *)
Eval simpl in increment (increment (increment ZERO)). (** 3 *)
Eval simpl in increment (increment (increment (increment ZERO))). (** 4 *)
Example bin_test1: unary (increment (increment ZERO)) = unary(increment (ZERO)) + 1.
Proof. simpl. reflexivity. Qed.

Example bin_test2: unary (increment (increment (increment ZERO))) = unary(increment (increment ZERO)) + 1.
Proof. simpl. reflexivity. Qed.
...
(* $Date: 2013-01-13 11:35:40 -0500 (Sun, 13 Jan 2013) $ *)
