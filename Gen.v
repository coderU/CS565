...
Proof.
   ...
   Case "l <> nil".
    intros n H.
    simpl. simpl in H. rewrite <- H.  apply IHt. reflexivity.
Qed.
...
Proof.
  ...
  generalize dependent n.
  induction l as [| h t].
    destruct n as [| n']. simpl. reflexivity. intros H. inversion H.
  simpl. intros n H.
  assert (length (snoc t v) = n).
    rewrite <- H. apply IHt. reflexivity.
